...
- If the temporary PowerShell helper cannot write the `UserChoice`/`UserChoiceLatest` values, SFTA falls back to direct writes so association changes are applied instead of failing.
